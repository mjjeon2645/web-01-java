...
class AccountTest {
  ...
  void transfer() {
    ...
    int oldSenderTransactionsCount = sender.transactionCount();
    int oldReceiverTransactionsCount = receiver.transactionCount();
    sender.transfer(receiver, 1000);

    assertEquals(2000, sender.amount());
    assertEquals(1500, receiver.amount());

    int newSenderTransactionsCount = sender.transactionCount();
    int newReceiverTransactionsCount = receiver.transactionCount();
